...
export const Header: React.FC = () => {
  const { currentPage, darkMode, toggleDarkMode, alertConfig } = useAppStore()
  const { ingredients, recipes } = useDataStore()

  // Calcular alertas de stock memoizado
  const lowStockAlert = useMemo(() => {
    const lowStock = new Set<string>()
    ingredients?.filter(i => i.deletedAt === null && i.stock <= (i.reorderThreshold ?? 0)).forEach(i => lowStock.add(i.id))

    const activeRecipes = recipes?.filter(r => r.deletedAt === null) || []
    for (const recipe of activeRecipes) {
      const tempConfig = { targetBatchLiters: alertConfig.targetBatchLiters, minStockPercentage: alertConfig.minStockPercentage }
      const { available, unavailable } = calculateRecipeAvailability(ingredients, [recipe], tempConfig)
      const checks = ((unavailable[0] as any) || (available[0] as any))?.ingredientsCheck || []
      if ((available.length === 0) && checks) checks.filter((c: any) => c.missing > 0).forEach((c: any) => lowStock.add(c.ingredientId))
    }

    return ingredients?.filter(i => lowStock.has(i.id)) || []
  }, [ingredients, recipes, alertConfig])

  const hideNav = currentPage && currentPage.startsWith(ROUTES.RECIPE_FORM.split(':')[0])
  const { logout } = useAuth() || {}
  const user = useUserStore(state => state.user)
  // Log de usuario obtenido del backend
  React.useEffect(() => {
    if (user) {
      console.log('[Header] Usuario persistente:', user);
    }
  }, [user]);
  const navigate = useNavigate()

  const handleBrewPilotClick = () => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
        ...
        {user && !hideNav && (
          <nav className="flex items-center space-x-2">
            ...
            <NavItem page={ROUTES.AVAILABLE} label="Recetas" />
...
console.log('ROUTES en Header:', ROUTES) // Log para verificar rutas
...
const StockAlertIcon: React.FC<StockAlertIconProps> = ({ lowStockAlert = [] }) => {
  ...
  const handleAlertClick = () => {
    // Si ya estamos en /ingredients, forzar el estado para que InventoryPage lo detecte
